...
uint8_t eeprom_read(uint32_t mapped_addr);
  ...
  #if BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_WIRELESS_PAPER
...
uint8_t boot_vector = 0x00;
  ...
  void boot_seq() { }
		...
		void led_tx_off() { digitalWrite(pin_led_tx, HIGH); }
			...
			void led_tx_off() { digitalWrite(pin_led_tx, LOW); }
		#endif
	#elif BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_WIRELESS_PAPER
			void led_rx_on()  { digitalWrite(pin_led_rx, HIGH); }
			void led_rx_off() {	digitalWrite(pin_led_rx, LOW); }
			void led_tx_on()  { digitalWrite(pin_led_tx, HIGH); }
			void led_tx_off() { digitalWrite(pin_led_tx, LOW); }
...
bool eeprom_product_valid() {
	...
	if (rval == PRODUCT_RNODE || rval == BOARD_RNODE_NG_20 || rval == BOARD_RNODE_NG_21 || rval == PRODUCT_HMBRW || rval == PRODUCT_TBEAM || rval == PRODUCT_T32_10 || rval == PRODUCT_T32_20 || rval == PRODUCT_T32_21 || rval == PRODUCT_H32_V2 || rval == PRODUCT_H32_V3 || rval == PRODUCT_WIRELESS_PAPER) {
	#elif PLATFORM == PLATFORM_NRF52
